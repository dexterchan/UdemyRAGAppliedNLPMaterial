...
def self_consistency_chain_of_thought(user_query: str, number_of_chains: int = 3):
    ...
    res_chains: list = [
        chain_of_thought(user_query=user_prompt) for _ in range(number_of_chains)
    ]

    res_concat = ";".join(res_chains)

    self_consistency_prompt = f"You will get multiple answers in <<>>, separated by ; <<{res_concat}>> Extract only the final equation and return the most common equation as it was provided originally"
